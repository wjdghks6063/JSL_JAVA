...
//		pds.t_gubun.value="update"; //request.getparameter로 받을수 없다. mpr로 받기 때문에 //해결 방법은 저번에 했던것처럼 saveattach로 따로 받던가 ?로 붙여서 보내준다.
		pds.t_no.value= no;
		pds.method="post";
		pds.action="Pds?t_gubun=update"; //?로 붙여서 보내주면 request.getparameter로 받을수 있다.
		pds.submit();
	}
	  ...
	  <!-- action을 처리하기전에 check()사용자 함수를 실행하고 되돌아 와라-->
			<table class="bord_table">
				...
				<tbody>
					...
					<tr>
						<th>내용</th>
						<td><textarea style="resize:none;" type="text" name="t_content" value="">${t_dto.getContent()}</textarea></td>
					</tr>
					<tr>
						<th>첨부파일</th>
						<td>
							<c:if test="${t_dto.getAttach() != null }">
								${t_dto.getAttach()}							
							&nbsp; 삭제 <input style="zoom:0.5" type="checkbox" name="del_checkbox_file" value="${t_dto.getAttach()}" class="inputcheck"><br>
							</c:if>
			...
			<div class="btn_wrap">
				<input type="button" onClick="javascript:goUpdate('${t_dto.getNo()}')" value="수정" class="btn_ok" style="cursor:pointer">&nbsp;&nbsp;
